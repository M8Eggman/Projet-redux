...
import { faMinus, faMotorcycle, faPlus, faXmark } from "@fortawesome/free-solid-svg-icons";
...
export default function PizzaPanier({ changeStyle }) {
  ...
  return (
    <>
      ...
      {!changeStyle && (
        <div className="panierDivBtnResponsive">
          <button className="panierBtnResponsive" onClick={() => setAffichePanier(true)}>
            <span>{quantite}</span>
            <span>Afficher le panier d'achat</span>
            <span>€{totalAvecLivraison.toFixed(2).replace(".", ",")}</span>
          </button>
        </div>
      )}
